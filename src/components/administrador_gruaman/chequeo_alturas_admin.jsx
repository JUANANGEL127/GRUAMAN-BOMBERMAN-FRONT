...
function ChequeoAlturasAdmin() {
  ...
  const handleBuscar = async () => {
    setLoading(true);
    try {
      const res = await axios.post("http://localhost:3000/chequeo_alturas_admin/buscar", filters);
  ...
  const handleDescargar = async (tipo) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:3000/chequeo_alturas_admin/descargar`,
      ...
      link.href = url;
      link.setAttribute("download", `chequeo_alturas.${tipo === "excel" ? "xlsx" : "pdf"}`);
