...
function App() {
  ...
  const handleObraChange = e => {
    ...
    const obraObj = listaObras.find(o => o.nombreobra === nombreObra); // <-- CAMBIO: nombreobra
    if (obraObj) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const resp = await axios.post("http://localhost:3000/validar-ubicacion", {
        obra_id: obraIdSeleccionada, // <-- CAMBIO: obra_id en vez de obraId
        lat: ubicacion.lat,
        lon: ubicacion.lon
      });
  ...
  const obrasFiltradas = (Array.isArray(listaObras) ? listaObras : []).filter(
    obra => obra && obra.nombreobra && obra.nombreobra.toLowerCase().includes(obraBusqueda.toLowerCase()) // <-- CAMBIO: nombreobra
  );
